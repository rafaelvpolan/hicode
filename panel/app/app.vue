...
const runsByCard = computed(() => {
  const m: Record<string, { count: number; tokens: number; cost: number }> = {}
  for (const r of runs.value) {
    const id = String(r.id)
    if (!m[id]) m[id] = { count: 0, tokens: 0, cost: 0 }
    m[id].count++
...
function renderChart() {
  ...
  const vals = data.map(r => Number(r.tokens_total) || 0)
  const maxIdx = vals.indexOf(Math.max(...vals, 0))
  const bars = vals.map((v, i) => ({ value: v, itemStyle: { color: i === maxIdx && v > 0 ? '#d29922' : '#2f81f7', borderRadius: [4, 4, 0, 0] } }))
  chart.setOption({
    backgroundColor: 'transparent',
    grid: { left: 56, right: 16, top: 20, bottom: 78 },
    tooltip: {
      trigger: 'axis',
      backgroundColor: '#161b22', borderColor: '#30363d', textStyle: { color: '#e6edf3' },
      formatter: (p: any[]) => {
        const i = p[0].dataIndex; const r = data[i]
        return `<b>${escTip(r.title)}</b><br/>bloco: ${escTip(String(r.ts).replace('T', ' ').replace('Z', ''))}<br/>tokens: ${Number(r.tokens_total).toLocaleString('pt-BR')}<br/>custo: $${escTip(r.cost_usd || '?')}`
      },
    },
    xAxis: { type: 'category', data: labels, axisLabel: { color: '#8b949e', rotate: 45, fontSize: 10 }, axisLine: { lineStyle: { color: '#30363d' } } },
    yAxis: { type: 'value', name: 'tokens', nameTextStyle: { color: '#8b949e' }, axisLabel: { color: '#8b949e' }, splitLine: { lineStyle: { color: '#21262d' } } },
    series: [{ type: 'bar', data: bars, barMaxWidth: 40, label: { show: true, position: 'top', color: '#e6edf3', fontSize: 10, formatter: (p: any) => p.value ? Number(p.value).toLocaleString('pt-BR') : '' } }],
  })
}
...
<template>
  ...
  <main class="wrap">
    ...
    <section class="card-sec">
      <h2>Tokens por bloco de execução</h2>
      <div v-show="runs.length" ref="chartEl" class="chart"></div>
      <div v-if="!runs.length" class="empty">nenhuma execução ainda — clique ▶ Começar num card.</div>
    </section>
    ...
    <section class="card-sec">
      ...
      <div v-for="c in state.cards" :key="c.id" class="erow" :class="{ high: c.risk === 'high' }">
        ...
        <div class="ecount">🔢 {{ (runsByCard[c.id]?.tokens ?? (Number(c.tokens_total) || 0)).toLocaleString('pt-BR') }} tokens · {{ runsByCard[c.id]?.count || 0 }} execução(ões)<template v-if="runsByCard[c.id]?.cost"> · ${{ runsByCard[c.id].cost.toFixed(4) }}</template></div>
      </div>
    </section>
  </main>
...
.tag{ color:var(--mut); font-size:12px } .navwrap button{ margin-left:auto }
