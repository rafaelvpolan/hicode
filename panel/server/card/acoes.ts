...
import { STATUS_URL_APROVADA, aguardaAprovacaoDeUrl, paraCardStatus, podeReexecutarEtapa } from '#shared/status'
import type { Fields } from './tipos'
import { setObjetivo, tituloDe } from './texto'
import { isoNow, slugify } from './util'
import { cardsDir } from '../motor/ambiente'
import { createCard, readCard, updateCard } from './armazenamento'
import { findCardFile } from './arquivos'
import { readClarify, writeClarify } from './clarify'
...
export function resumeFrom(id: string, step: string): ActionResult {
  ...
  if (!card) return null
  if (!podeReexecutarEtapa(paraCardStatus(card.fm.status))) return null
  if (!card.fm.worktree || !existsSync(join(card.fm.worktree, '.git'))) return null
  return updateCard(id, {
    fields: { resume_from: step, status: STATUS_URL_APROVADA },
    log: fm => `${isoNow()} ${fm.status || 'INBOX'}->${STATUS_URL_APROVADA} replay a partir de ${step}`,
  })
}
...
export function requestCorrection(id: string, file: string, instruction: string, line = '', lineText = ''): ActionResult {
  const card = readCard(id)
  if (!card) return null
  if (!aguardaAprovacaoDeUrl(paraCardStatus(card.fm.status))) return null
  if (!card.fm.worktree || !existsSync(join(card.fm.worktree, '.git'))) return null
