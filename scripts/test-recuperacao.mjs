...
try {
  ...
  await card.getByText(/configuracao original nao foi comprovada/i).waitFor()
  assert.equal(await card.getByRole('button', { name: 'Confirmar revalidacao da etapa', exact: true }).count(), 0)
  await card.getByText('Configuracoes registradas', { exact: true }).click()
  const configuracoes = card.getByLabel('Configuracao a restaurar')
  await configuracoes.selectOption({ index: 1 })
  await card.getByRole('button', { name: 'Restaurar esta configuracao', exact: true }).click()
